    ...
    try {
      ...
      if ((articleUrl && check == null) || undefined) {
        ...
        let articleTitle = await articlePage
          .locator('h1[class="o-hero-news__title f-title-3"]')
          .textContent();
        let articleDate = await articlePage.locator("time").textContent();
        let articleAuthor = "not specified";
        try {
          articleAuthor = await articlePage
            .locator('span[class="o-hero-article__authors"]')
            .textContent({ timeout: 1000 });
        } catch (error) {
          console.log(error);
        }
        let articleText = await articlePage
          .locator('div[class="o-page__content"]')
          .textContent();
