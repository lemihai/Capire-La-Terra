...
// Main function
export async function theguardianScraper(URL: string, page: any, browser: any) {
  //  trimming the url according to the woodcentral template
  const baseURL = URL.split(".com")[0] + ".com";
  await page.goto(URL);

  let pages = await page
    .locator('main[data-layout="FrontLayout"]')
    .locator("a")
    .all();
    ...
    try {
      ...
      if (articleUrl && check?.length === 0) {
        ...
        let articleTitle = await articlePage
          .locator('div[style="--grid-area:headline;"]')
          .locator("h1")
          .textContent({ timeout: 1000 });
        let articleDate = articleUrl.split(".com")[1];
        articleDate = articleDate.split("/");
        articleDate =
          articleDate[4] + " " + articleDate[3] + " " + articleDate[2];
        let articleAuthor = "Not defined";
        try {
